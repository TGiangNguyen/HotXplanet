...
xstart = dthetadeg0
Pmax = 1.5*press_vap_calc(Tf0)
Pmin = Pmax/10
sigfig_cap = runparams['sigfig_cap']
Pdiff_cap = Pmax/(10**sigfig_cap)
...
# initial variables (T0 = Tf0 only works for transparent atmosphere)
V0 = 0
T0 = Tf0
Q0 = 0
...
if time0[4]<10:
    timestring = '{run}-output-{mon}-{day}-{hr}0{min}'.format(run=run_name,mon=time0[1],day=time0[2],hr=time0[3],min=time0[4])
else:
    timestring = '{run}-output-{mon}-{day}-{hr}{min}'.format(run=run_name,mon=time0[1],day=time0[2],hr=time0[3],min=time0[4])

# write a output text file to confirm
outfile = open('output.txt','w')
outfile.write(run_name)
outfile.write('\n'+timestring)
outfile.write('\n'+'dtheta = {n} deg'.format(n=runparams['dthetadeg']))
outfile.write('\n'+'RT factor = {n}/1'.format(n=runparams['RT_factor']))
outfile.write('\n'+'Latent heat factor = {n}/1'.format(n=runparams['RT_factor']))
outfile.close()
